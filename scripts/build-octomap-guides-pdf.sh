...
build_pdf() {
  ...
  printf 'OCTOMAP_GUIDE_PDF_PASS file=%s pages=%s sha256=%s\n' \
...
build_pdf \
  ...
  "${output_directory}/TUTORIAL_UNITREE_L1_OCTOMAP_MAPPING.pdf" \
  7 12 \
  'static_sensor:=true' \
...
build_pdf \
  ...
  "${output_directory}/RAPPORT_CONFIGURATION_UNITREE_L1_OCTOMAP.pdf" \
  7 12 \
  'catkinConfig.cmake' \
  'l1_octomap_bringup' \
