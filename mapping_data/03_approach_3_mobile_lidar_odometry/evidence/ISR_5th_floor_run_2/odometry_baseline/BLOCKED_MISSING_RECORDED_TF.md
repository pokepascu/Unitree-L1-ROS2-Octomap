...
The project constraint is explicit: `base_link -> unilidar_lidar` translation is exactly `(0, 0, 0) m`.
...
The repository includes a zero-translation hand-eye calibration pipeline that estimates this rotation independently from recorded robot odometry and KISS-ICP relative motions on HcMR, ISR run 1 and ISR run 2, and accepts it only if the three estimates are physically consistent.
